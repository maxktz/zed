...
use ui::prelude::*;
use workspace::{
    MultiWorkspace, OpenMode, PreviousWorkspaceState, ProjectGroupKey, ToastView, Workspace,
    WorkspaceSlotId, dock::DockPosition,
};
...
async fn open_worktree_workspace(
    ...
    let (workspace_task, modal_workspace, opening_slot_id) =
        window_handle.update(cx, |multi_workspace, window, cx| {
            ...
            let opening_slot_id = WorkspaceSlotId::new(
                project_group_key.clone().unwrap_or_else(|| {
                    ProjectGroupKey::new(remote_connection_options.clone(), path_list.clone())
                }),
                path_list.clone(),
            );
            multi_workspace.start_workspace_slot_operation(opening_slot_id.clone(), cx);
            ...
            (task, modal_workspace, opening_slot_id)
        })?;
    ...
    window_handle
        .update(cx, |multi_workspace, _window, cx| {
            multi_workspace.finish_workspace_slot_operation(opening_slot_id, cx);
        })
        .ok();
